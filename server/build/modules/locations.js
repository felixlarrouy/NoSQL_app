...
exports.getLocations = callback => {
  mongodb.getDocuments().then(docs => {
    ...
    for (var i = 0; i < docs.length; i++) {
      promises.push(locate(docs[i]._id, "https://maps.googleapis.com/maps/api/geocode/json?address=" + docs[i].restaurant.buildingnum + "+" + docs[i].restaurant.street + "+" + docs[i].restaurant.borough + "&key=AIzaSyD0XB3hdvsrJnTEjRawEJdismD34dKq8Js"));
    }
...
function locate(_id, url) {
  return new Promise(function (resolve, reject) {
    request({
      ...
      console.log(_id);
      resolve([_id, apiResponse.results[0].formatted_address, apiResponse.results[0].geometry.location.lat, apiResponse.results[0].geometry.location.lng]);
    });
  });
}